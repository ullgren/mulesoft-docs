...
You should declare your input and output metadata specifying the shape of event you require and what is the on of the event you return to the caller. Output event declaration, if not specified, is automatically inferred from flow content when the flow is not recursive.
